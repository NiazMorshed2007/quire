...
const TasksPage: FC<Props> = ({ tabs, sublists, type }) => {
  ...
  return (
    <div className="task-page overflow-auto custom-scrollbar position-relative">
      <header className="d-flex align-items-center justify-content-between position-fixed">
        <Dropdown
          overlay={
            <Menu>
              <Menu.Item icon={<FiPieChart />}>Active Tasks</Menu.Item>
              <Menu.Item icon={<BsListTask />}>All Tasks</Menu.Item>
              <Menu.Item icon={<AiOutlineUser />}>My Tasks</Menu.Item>
              <Menu.Item icon={<FiUsers />}>Group by assigne</Menu.Item>
              <Menu.Item icon={<BsCalendar3 />}>Group by date</Menu.Item>
              <Menu.Divider />
              <Menu.Item icon={<FiSettings />}>Customize</Menu.Item>
            </Menu>
          }
          trigger={["click"]}
        >
      ...
      <div className="main-wrapper pt-5">
        <div className="inner-main">
          ...
          {currentView === "board" && (
            <Board
              tasks={tasks && tasks}
              statuses={sublist ? sublist.statuses : listTab.statuses}
            />
          )}
        </div>
      </div>
